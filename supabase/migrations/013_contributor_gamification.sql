...
ADD COLUMN IF NOT EXISTS contributed_by_product_user_id UUID REFERENCES product_users(id) ON DELETE
SET NULL;
ALTER TABLE knowledge_documents
...
ALTER TABLE knowledge_documents
ADD COLUMN IF NOT EXISTS original_user_document_id UUID REFERENCES user_documents(id) ON DELETE
SET NULL;
ALTER TABLE knowledge_documents
ADD COLUMN IF NOT EXISTS source_label TEXT DEFAULT 'upload';
CREATE INDEX IF NOT EXISTS idx_knowledge_docs_contributor ON knowledge_documents(contributed_by_product_user_id)
...
ALTER TABLE product_users
ADD COLUMN IF NOT EXISTS contributor_rank TEXT DEFAULT 'newcomer';
ALTER TABLE product_users
ADD COLUMN IF NOT EXISTS total_docs_contributed INTEGER DEFAULT 0;
ALTER TABLE product_users
ADD COLUMN IF NOT EXISTS total_chunks_contributed INTEGER DEFAULT 0;
ALTER TABLE product_users
...
CREATE INDEX IF NOT EXISTS idx_product_users_score ON product_users(contributor_score DESC)
WHERE contributor_score > 0;
-- 3. VIEWS & FUNCTIONS
-- Leaderboard View
DROP VIEW IF EXISTS knowledge_leaderboard;
CREATE OR REPLACE VIEW knowledge_leaderboard AS
SELECT pu.id AS product_user_id,
    pu.display_name,
    pu.product_id,
...
BEGIN -- Get current stats
SELECT * INTO v_current
FROM product_users
WHERE id = p_product_user_id;
IF NOT FOUND THEN RETURN json_build_object('success', false);
END IF;
-- Calc bonuses
...
IF v_is_consecutive THEN v_streak_bonus := LEAST(
    (COALESCE(v_current.contribution_streak, 0) + 1) * 15,
    150
);
END IF;
...
SET contributor_score = v_new_score,
...
v_admin_name TEXT;
BEGIN -- Get suggestion details
SELECT * INTO v_suggestion
FROM knowledge_suggestions
WHERE id = p_suggestion_id;
IF NOT FOUND THEN RAISE EXCEPTION 'Suggestion not found';
END IF;
-- Get contributor info if exists
IF v_suggestion.source_user_id IS NOT NULL THEN
SELECT id,
    display_name AS name,
    NULL as email INTO v_contributor
FROM product_users
WHERE id = v_suggestion.source_user_id;
END IF;
-- Get admin name
IF p_admin_id IS NOT NULL THEN
SELECT email INTO v_admin_name
FROM auth.users
WHERE id = p_admin_id;
END IF;
-- Loop through target KBs
FOREACH v_kb_id IN ARRAY p_knowledge_base_ids LOOP -- Create Knowledge Document
...
$$;
-- Grant permissions
GRANT SELECT ON knowledge_leaderboard TO authenticated,
    service_role;

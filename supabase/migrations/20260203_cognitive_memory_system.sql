...
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS entities_mentioned JSONB DEFAULT '[]';
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS primary_entities JSONB DEFAULT '[]';
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS intent_type VARCHAR(50);
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS intent_summary TEXT;
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS dominant_sentiment VARCHAR(30);
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS sentiment_journey JSONB DEFAULT '[]';
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS resolution_status VARCHAR(20) DEFAULT 'ongoing';
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS user_insights JSONB DEFAULT '{}';
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS expertise_signals JSONB DEFAULT '{}';
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS action_items JSONB DEFAULT '[]';
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS follow_up_needed BOOLEAN DEFAULT false;
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS follow_up_topic TEXT;
ALTER TABLE chat_sessions
...
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS thinking_time_total INTEGER DEFAULT 0;
ALTER TABLE chat_sessions
...
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS continued_from_session_id UUID;
ALTER TABLE chat_sessions
ADD COLUMN IF NOT EXISTS continuation_context TEXT;
...
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS query_rewrite_reason TEXT;
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS source_count INTEGER DEFAULT 0;
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS citations_used JSONB DEFAULT '[]';
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS confidence FLOAT;
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS reasoning_enabled BOOLEAN DEFAULT false;
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS reasoning_data JSONB;
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS reasoning_duration_ms INTEGER;
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS crag_verdict VARCHAR(20);
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS crag_confidence FLOAT;
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS entities_mentioned JSONB DEFAULT '[]';
ALTER TABLE chat_messages
...
ALTER TABLE chat_messages
ADD COLUMN IF NOT EXISTS formatting_used JSONB DEFAULT '[]';
ALTER TABLE chat_messages
...
CREATE TABLE IF NOT EXISTS memory_facts (
    ...
    product_user_id UUID REFERENCES product_users(id) ON DELETE CASCADE,
    -- Source Tracking
    source_session_id UUID REFERENCES chat_sessions(id) ON DELETE
    SET NULL,
        source_message_id UUID REFERENCES chat_messages(id) ON DELETE
    SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_facts_user ON memory_facts(
    product_user_id,
    is_active,
...
CREATE POLICY "Users can view own sessions" ON chat_sessions FOR
SELECT USING (
        product_user_id IN (
            SELECT id
            FROM product_users
...
CREATE POLICY "Users can insert own sessions" ON chat_sessions FOR
INSERT WITH CHECK (
        product_user_id IN (
            SELECT id
            FROM product_users
...
CREATE POLICY "Users can update own sessions" ON chat_sessions FOR
UPDATE USING (
        product_user_id IN (
            SELECT id
            FROM product_users
...
CREATE POLICY "Users can view own messages" ON chat_messages FOR
SELECT USING (
        session_id IN (
            SELECT id
            FROM chat_sessions
...
CREATE POLICY "Users can insert own messages" ON chat_messages FOR
INSERT WITH CHECK (
        session_id IN (
            SELECT id
            FROM chat_sessions
...
DROP POLICY IF EXISTS "Users can view own profile" ON user_cognitive_profile;
CREATE POLICY "Users can view own profile" ON user_cognitive_profile FOR
SELECT USING (
        product_user_id IN (
            SELECT id
            FROM product_users
...
DROP POLICY IF EXISTS "Users can update own profile" ON user_cognitive_profile;
CREATE POLICY "Users can update own profile" ON user_cognitive_profile FOR
UPDATE USING (
        product_user_id IN (
            SELECT id
            FROM product_users
...
DROP POLICY IF EXISTS "Users can view own entities" ON user_entity_graph;
CREATE POLICY "Users can view own entities" ON user_entity_graph FOR
SELECT USING (
        product_user_id IN (
            SELECT id
            FROM product_users
...
DROP POLICY IF EXISTS "Users can manage own entities" ON user_entity_graph;
CREATE POLICY "Users can manage own entities" ON user_entity_graph FOR ALL USING (
    product_user_id IN (
        SELECT id
        FROM product_users
        WHERE user_id = auth.uid()
...
DROP POLICY IF EXISTS "Users can view own facts" ON memory_facts;
CREATE POLICY "Users can view own facts" ON memory_facts FOR
SELECT USING (
        product_user_id IN (
            SELECT id
            FROM product_users
...
DROP POLICY IF EXISTS "Users can manage own facts" ON memory_facts;
CREATE POLICY "Users can manage own facts" ON memory_facts FOR ALL USING (
    product_user_id IN (
        SELECT id
        FROM product_users
        WHERE user_id = auth.uid()
...
DROP POLICY IF EXISTS "Users can view own insights" ON proactive_insights;
CREATE POLICY "Users can view own insights" ON proactive_insights FOR
SELECT USING (
        product_user_id IN (
            SELECT id
            FROM product_users
...
DROP POLICY IF EXISTS "Users can manage own insights" ON proactive_insights;
CREATE POLICY "Users can manage own insights" ON proactive_insights FOR ALL USING (
    product_user_id IN (
        SELECT id
        FROM product_users
        WHERE user_id = auth.uid()
...
CREATE TRIGGER update_user_cognitive_profile_updated_at BEFORE
UPDATE ON user_cognitive_profile FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
DROP TRIGGER IF EXISTS update_user_entity_graph_updated_at ON user_entity_graph;
CREATE TRIGGER update_user_entity_graph_updated_at BEFORE
UPDATE ON user_entity_graph FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
DROP TRIGGER IF EXISTS update_memory_facts_updated_at ON memory_facts;
CREATE TRIGGER update_memory_facts_updated_at BEFORE
UPDATE ON memory_facts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
-- ============================================================================
-- DONE!
-- ============================================================================
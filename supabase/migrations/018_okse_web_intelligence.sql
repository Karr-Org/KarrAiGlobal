...
CREATE TABLE IF NOT EXISTS trusted_web_sources (
    ...
    domain TEXT NOT NULL,                              -- e.g., "cbic.gov.in"
    display_name TEXT,                                 -- e.g., "CBIC Official"
    url_patterns TEXT[] DEFAULT '{}',                  -- e.g., ["/circulars/*"]
    exclude_patterns TEXT[] DEFAULT '{}',              -- URLs to skip
    
    -- Authority & trust
    authority_score INTEGER DEFAULT 5 CHECK (authority_score BETWEEN 1 AND 10),
    source_type TEXT DEFAULT 'official',               -- official, professional, commentary, blog
    
    -- Crawling configuration
    crawl_frequency TEXT DEFAULT 'daily',              -- realtime, 15min, hourly, daily, weekly
    css_selectors JSONB DEFAULT '{}',                  -- {"content": ".main-content", "title": "h1"}
    use_javascript BOOLEAN DEFAULT FALSE,              -- Use headless browser
    ...
    last_crawl_status TEXT,                            -- success, error, rate_limited
...
CREATE TABLE IF NOT EXISTS web_knowledge_cache (
    ...
    -- Content tracking
    content_hash TEXT,                                 -- MD5 for delta detection
    raw_content TEXT,                                  -- Original extracted text
    
    -- Metadata
    page_type TEXT,                                    -- circular, notification, article, etc.
    extracted_date DATE,                               -- Date mentioned in content
    extracted_entities JSONB DEFAULT '{}',             -- Extracted entities (sections, forms, etc.)
    ...
    -- Tracking
    times_used INTEGER DEFAULT 0,                      -- How often cited in responses
...
CREATE TABLE IF NOT EXISTS web_knowledge_chunks (
    ...
    source_domain TEXT NOT NULL,                       -- "cbic.gov.in" for citations
    source_display_name TEXT,                          -- "CBIC Official"
    source_url TEXT NOT NULL,                          -- Full URL for linking
...
CREATE TABLE IF NOT EXISTS semantic_cache (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    product_id UUID NOT NULL REFERENCES products(id) ON DELETE CASCADE,
    user_id UUID REFERENCES auth.users(id),            -- Optional: user-specific caching
    
    -- Query
    query_text TEXT NOT NULL,
    query_normalized TEXT NOT NULL,                    -- Lowercase, trimmed, etc.
    query_embedding vector(768),
    
    -- Response
    response TEXT NOT NULL,
    sources JSONB DEFAULT '[]',                        -- Sources used in response
    reasoning_metadata JSONB DEFAULT '{}',             -- CRAG verdict, confidence, etc.
    
    -- Quality metrics
    confidence FLOAT,
    complexity_level TEXT,                             -- SIMPLE, MODERATE, COMPLEX, MULTI_HOP
...
RETURNS TABLE (
    chunk_id UUID,
    content TEXT,
    source_type TEXT,              -- 'kb' or 'web'
    source_domain TEXT,            -- null for KB, domain for web
    source_title TEXT,
    source_url TEXT,
    authority_score INTEGER,
    contextual_summary TEXT,
...
-- ============================================================================
-- TRIGGER: Increment cache hit count
-- ============================================================================

...
export async function GET(request: Request) {
    ...
    if (error) {
        console.error('[Facebook Callback] Auth error:', error, errorDescription);
    ...
    try {
        ...
        await saveAccount(userId, 'facebook', {
        ...
        console.log('[Facebook Callback] Account connected successfully for user:', userId, productId ? `(product: ${productId})` : '(user-level)');

        const separator = redirectBase.includes('?') ? '&' : '?';
        return NextResponse.redirect(
            new URL(`${redirectBase}${separator}connected=facebook`, request.url)
        );
    } catch (err) {

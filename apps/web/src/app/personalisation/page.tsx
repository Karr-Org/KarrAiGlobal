    ...
    useEffect(() => {
        async function init() {
            try {
                ...
                // Get the first product user (for demo, we use the first product)
                ...
                if (productUsers && productUsers.length > 0) {
                    setProductUser(productUsers[0]);

                    // Get product info
                    const { data: productData } = await supabase
                        .from('products')
                        .select('id, name, slug')
                        .eq('id', productUsers[0].product_id)
                        .single();
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-cream-50">
                <div className="text-center">
                    ...
                    <Link href="/auth/login" className="text-terracotta-600 hover:underline text-sm">
                        Sign in to continue
                    </Link>
                </div>
            </div>
        ...
        <div className="h-screen bg-cream-50 flex overflow-hidden">
            ...
            <div className={`${sidebarOpen ? 'w-[280px]' : 'w-16'} flex-shrink-0 transition-all duration-300 bg-cream-50 flex flex-col overflow-hidden`}>
                ...
                <div className="px-3 pb-2">
                    <button
                        onClick={() => router.push(`/p/${product.slug}/dashboard`)}
                ...
                <div className="px-2 space-y-1">
                    ...
                    {/* My Clients */}
                    <button
                        onClick={() => router.push(`/p/${product.slug}/dashboard?tab=clients`)}
